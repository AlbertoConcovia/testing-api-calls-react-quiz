...
describe("App.test.js - API testing", () => {
  ...
  test("Check the first title has been rendered correctly in the document", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Films from Studio Ghibli API/i)
      ).toBeInTheDocument()
    );
  });
...
Then error message should be present`, async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Oops… something went wrong, try again/i)
      ).toBeInTheDocument()
    );
  });
...
Then error message should be present`, async () => {
    ...
    render(<App {...requiredProps} />);
    await waitFor(() =>
      expect(screen.getByText(/I'm a tea pot 🫖, silly/i)).toBeInTheDocument()
    );
  });
});
